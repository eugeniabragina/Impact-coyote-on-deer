...
# male harvest in the Eastern US in 1981-2010 
# You will need 'Buck_harvest.csv to run this code
library(MuMIn)
library(lme4)
library(lmerTest)
library(nlme)
...
periods = c("1981", '1985', '1985', '1990', '1990', '1995', '1996', '2000', '2000', '2005', 
            '2005', '2010', '2011', '2014')
order = c(7,7,6,6,5,5,4,4,3,3,2,2,1)
 ...
 for (i in c(1,3,5,7,9,11,13)) {
  ...
  cd = read.table('Buck_harvest.csv', sep = ',', header = T, check.names = F)
  cd=cd[2-dim(cd)[1],]
  ...
  if (i<7) {
    m = lme(log(lam) ~ coyote_James + Pop.dens.2010 + BIO12_median + BIO15_median + 
...
m3 = lme(log(lam) ~ WOODY_WETL.1992 + 1, random = ~1|state, correlation = corExp(form =~xcoord+ycoord), data = cd, na.action = "na.fail")
AIC(m1, m2, m3)
# df       AIC
# 
# go with m
...
m3 = lme(log(lam) ~ pop.den + pop.growth, random = ~1|state, correlation = corExp(form =~xcoord+ycoord), data = cd, na.action = "na.fail")
...
plot(cd$lam ~ cd$BIO17_median)
par(mfrow = c(1,2))
hist(cd$coyote_James)
hist(cd$coyote_year_recl)

hist(scale(cd$coyote_James))
hist(scale(cd$coyote_year_recl))
plot(cd$lam ~scale(cd$coyote_year_recl))
m1 = lm(cd$lam ~scale(cd$coyote_year_recl))
plot(m1)
hist(log(cd$lam))
